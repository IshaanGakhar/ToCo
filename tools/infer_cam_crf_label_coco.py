...
parser.add_argument("--dst_dir", default='cam_label_crf', type=str, help="dst")
...
def crf_proc(args):
    ...
    txt_name = os.path.join(args.list_folder, args.eval_set) + '.txt'
    ...
    crf_pred_path = os.path.join(args.dst_dir, args.eval_set)
    ...
    def _job(i):
        ...
        image = robust_read_image(image_name).astype(np.float32)
        ...
        _pred = np.squeeze(pred_h).astype(np.uint8)
    ...
    score = scores(gts, preds, num_classes=81)
